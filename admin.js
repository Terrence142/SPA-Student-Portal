...
(function () {
  ...
  function renderApprovals() {
    ...
    rows.forEach(function (a) {
      ...
      // The roster check is what makes an approval safe: an account whose
      // Student ID is not in the roster cannot be let in.
      var rosterCell = a.onRoster
        ? '<span class="badge badge-green">Yes</span>' +
          '<span class="roster-name">' + escapeHtml(a.studentName || "") +
          (a.grade ? " · " + escapeHtml(a.grade) : "") + "</span>"
        : '<span class="badge badge-red">Not on file</span>';
      ...
      if (status === "pending") {
        ...
        approveBtn.disabled = !a.onRoster;
        if (!a.onRoster) {
          approveBtn.title = "This Student ID is not in the roster, so it cannot be approved.";
        }
        ...
        moveBtn.disabled = status !== "active" && !a.onRoster;
  ...
  function toggleReviewRow(tbody, tr, button, account) {
    ...
    cell.innerHTML =
      ...
      onFile +
      (account.onRoster
        ? '<p class="review-note review-ok">This Student ID is on the roster.</p>'
        : '<p class="review-note review-bad">This Student ID is not on the roster — the account cannot be approved.</p>') +
      "</div></div>";
