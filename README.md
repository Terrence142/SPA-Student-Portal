...
persistence to the Excel file, and path-traversal protection. It backs up and
restores your `database.xlsx`, so it is safe to run on real data.

> The suite asserts against a **fresh** seeded database, so several checks
expect the sample students (`Maria Santos` and friends) and the default
passwords. On a database with your own data those particular checks will
report a mismatch even though the app is fine. Run it on a copy, or against a
deleted `database.xlsx`, for a clean result.
